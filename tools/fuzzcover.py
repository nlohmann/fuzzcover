...
def reduce_files_additive(quiet=False):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with tqdm(total=len(files), unit="files", leave=False, desc='pass 1 (additive)   ', disable=quiet) as pbar:
            for file in files:
...
def reduce_files_subtractive(quiet=False):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with tqdm(total=len(files), unit="files", leave=False, desc='pass 2 (subtractive)', disable=quiet) as pbar:
            for file in files:
...
def reduce_file_length(quiet=False):
    ...
    with tqdm(total=corpus_size().bytes, unit="bytes", leave=False, desc='pass 3 (size)       ', disable=quiet) as pbar:
...
def dump(filename=None):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if filename:
            print('Saved corpus to {filename}.'.format(filename=cf.bold_white(filename)))
...
def fuzz(max_total_time=0, runs=-1, only_ascii=False, max_len=0, quiet=False):
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            env = dict(os.environ, LLVM_PROFILE_FILE=os.path.join(temp_dir, 'default.profraw'))
...
def fuzz_menu():
    ...
    fuzz(max_total_time=int(answers['max_total_time']),
         runs=int(answers['runs']),
         only_ascii=answers['only_ascii'],
         max_len=int(answers['max_len']))
    ...
    questions = [
        {
            ...
            'choices': ['Start fuzzing', 'Reduce corpus', 'Dump corpus', 'Save corpus to JSON file', 'Show coverage',
                        'Clear corpus', 'I\'m feeling lucky', 'Quit']
        }
    ]
...
if __name__ == '__main__':
    ...
    parser.add_argument('corpus', metavar='CORPUS_DIRECTORY', type=str, nargs='?',
                        help='The directory of the corpus. If not provided, the name of the corpus directory will be '
                             ...
                             'exist.')
    parser.add_argument('--lucky', action='store_true', help='Create a test suite without user interaction.')
